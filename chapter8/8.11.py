# функции, которые печатают сообщение из копии списка и переносят его в новый список
def message_sender(messages, sent_messages):
    """выводит сообщения из списка"""
    while messages:
        # индекс 0, чтобы порядок содержимого в заполненном списке совпадал с изначальным
        message = messages.pop(0)
        print(message)
        sent_messages.append(message)


# сообщения для списка, сами списки и вызов функции
message_0 = 'Первое сообщение.'
message_1 = 'Второе сообщение.'
message_2 = 'Третье сообщение.'
message_list = [message_0, message_1, message_2]
messages_sent = []
message_sender(message_list[:], messages_sent)

# вызов списков, для проверки содержимого. в обоих списках должно быть идентичное содержимое
print('\nСписки и их содержимое:')
print(message_list)
print(messages_sent)
